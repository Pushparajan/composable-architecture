...
- Detailed capabilities of the Illustrator, Creative Production, Express and Content Tagging APIs.
- InDesign API specifics beyond its listed storage providers.

## LinkedIn kit

**Post**

The quickest way to misunderstand Firefly Services is to open the Firefly website.

That's the app. Firefly Services is the API side: REST APIs you call from your own code. No canvas, no login screen, no designer clicking Export.

And it isn't one API. Adobe's developer docs list ten families right now, including Firefly, Photoshop, InDesign, Lightroom, Audio/Video, Express and Content Tagging.

The split that matters most: Firefly makes pixels. It generates, expands, fills and varies images. It doesn't own layers, live text or page layout. That's Photoshop API and InDesign API territory.

So a pipeline usually looks like this. Something upstream fires an event. Firefly adapts the image. Photoshop API drops it into a layered template. Finished files land in your DAM.

If your architecture diagram has one box labeled "Firefly," it's worth redrawing before you build.

Which API would you wire in first?

**Alternate hooks**
- Firefly Services has no canvas, no login and no Export button. That's the point.
- If your architecture diagram has one box labeled "Firefly," there are probably three APIs hiding in it.

**First comment:** Docs home for the Firefly API: https://developer.adobe.com/firefly-services/docs/firefly-api/ Next up: the auth and storage details that break calls before anything renders.
