...
- The internal training method (I saw no Adobe documentation on it, so I've left it out).
- The exact error returned when an unshared model is called.
- The format of the technical account email address; use the value the Developer Console shows.

## LinkedIn kit

**Post**

Your custom Firefly model finished training. Your API call still can't use it.

The missing step is sharing. Adobe's docs say a trained model has to be shared with your technical account before the List Custom Models and image generation APIs can see it. In the Developer Console, open your project and copy the Technical Account Email. Then share the model with that address from Firefly.

After that, call the async generate endpoint with an x-model-version header for custom models, like image3_custom, and pass your customModelId in the body.

Back up a step, to training. Both subject and style models take 10 to 30 images. For a subject, keep it the same product and color, in clear focus, filling at least a quarter of the frame. Adobe suggests mixing plain and busy backgrounds, not only white ones.

For a style model, use images that share the look you want.

Subject model or style model first?

**Alternate hooks**
- Training a custom model is the easy part. Getting your API to see it is where people stall.
- A Technical Account Email looks like a random string. It's also the key to your custom model.

**First comment:** Granting apps access: https://developer.adobe.com/firefly-services/docs/firefly-api/guides/how-tos/cm-share-model/ Custom models overview: https://developer.adobe.com/firefly-services/docs/firefly-api/guides/concepts/custom-models/
