...
- That source and mask must have identical pixel dimensions (plausible, but I didn't find it stated).
- The exact path of the async Similar and Fill endpoints; use the API reference.
...
Three Firefly endpoints cover most of the adapting work.

Expand. Send the source and a target size, then position the original with placement or use a mask. A prompt is optional.

Fill. You send a source, a mask and a size. White in the mask means that area can change. Black stays put. Adobe's tutorial swaps employee headshot backgrounds this way.

Similar. Start from an approved image and ask for variations, instead of re-prompting and hoping the layout holds.

Two habits worth copying from Adobe's examples. Upload the source once and reuse the ID, then start several Expand jobs in parallel. And run one image end to end before you queue thousands, because mask polarity is easy to get backwards.

What's the biggest adaptation job on your plate right now?

**Alternate hooks**
- Most of a campaign's image work is resizing and swapping, not inventing. Firefly has endpoints for exactly that.
- Before you queue 10,000 Fill jobs, run one. Mask polarity has a way of surprising people.

**First comment:** Expand tutorial: https://developer.adobe.com/firefly-services/docs/firefly-api/guides/how-tos/firefly-expand-image-api-tutorial/ Fill tutorial: https://developer.adobe.com/firefly-services/docs/firefly-api/guides/how-tos/firefly-fill-image-api-tutorial Masking: https://developer.adobe.com/firefly-services/docs/firefly-api/guides/concepts/masking
