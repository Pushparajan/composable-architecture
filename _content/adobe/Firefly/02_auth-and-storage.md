...
- The exact status code for an expired token.
- Any per-minute rate limit numbers.

## LinkedIn kit

**Post**

Your Firefly access token lasts 24 hours. If it's sitting in an environment variable, that's a countdown.

Adobe's docs say each token is valid for 24 hours, and the response tells you so: expires_in 86399.

Two mistakes I'd steer you away from. Minting a new token on every call. And pasting one token into an env var and forgetting it.

Cache it, refresh it before it dies, and let a standard OAuth library do the work. (I like renewing an hour early. That's my habit, not an Adobe rule.)

The other thing that trips people up is files. Firefly doesn't want your bytes in the request. It wants a pre-signed URL, or an ID from its upload endpoint. For image workflows the listed domains are Amazon, Azure and Dropbox. Other Firefly Services APIs allow different lists, so check each one.

Upload IDs last 7 days, and uploads top out at 15 MB.

How do you share one token across workers?

**Alternate hooks**
- expires_in: 86399. That number is doing more work than it looks like.
- Before Firefly renders a single pixel, three boring things can stop your pipeline.

**First comment:** Authentication guide: https://developer.adobe.com/firefly-services/docs/firefly-api/getting-started/ Image upload and supported domains: https://developer.adobe.com/firefly-services/docs/firefly-api/guides/concepts/image-upload/
